# Product Leader — Shradha Tripathi

Interactive product management portfolio: career arc, impact ledger, metrics, and experience across fintech, AI/ML, and enterprise platforms.

**[View live portfolio ?](https://shradhatripathi-droid.github.io/Product-Leader/experience-detail_5.html)**

## Highlights

- 18+ years in product management (Intuit, Amazon, Flipkart, Deutsche Bank, The Hartford, CSC)
- $220M+ revenue and savings driven across career
- Staff PM, Technical — QuickBooks Payments Platform @ Intuit

## Contact

- Email: shradha.tripathi@gmail.com
- Location: Natick, MA
- GitHub: [shradhatripathi-droid/Product-Leader](https://github.com/shradhatripathi-droid/Product-Leader)

## GitHub Pages

This repo is set up for GitHub Pages. After pushing, enable **Settings ? Pages ? Deploy from branch ? `main` / root** to publish at `https://shradhatripathi-droid.github.io/Product-Leader/`.
